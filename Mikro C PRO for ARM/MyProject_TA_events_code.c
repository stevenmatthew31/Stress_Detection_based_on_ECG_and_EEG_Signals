...
//        TFT_Line((i_plot - 1), 300 - 70*(eeg_ptr->eeg_signal[i-1]), i_plot , 300 - 70*((eeg_ptr->eeg_signal[i])));
     if (i_plot >= 1) {

        TFT_Set_Pen(CL_TEAL, 2);
        TFT_Line((i_plot - 1), 230 - 10*(get_rotating_index( eeg_ptr->lpf_eeg, N_DISPLAY_EEG, i-1)), i_plot , 230 - 10*(get_rotating_index( eeg_ptr->lpf_eeg, N_DISPLAY_EEG, i)));

        TFT_Set_Pen(CL_WHITE, 2);
        TFT_Line((i_plot - 1), 270 - 10*(get_rotating_index( eeg_ptr->hpf_eeg, N_DISPLAY_EEG, i-1)), i_plot , 270 - 10*(get_rotating_index( eeg_ptr->hpf_eeg, N_DISPLAY_EEG, i)));
     }
     //F3
     if (i_plot >= 1) {
